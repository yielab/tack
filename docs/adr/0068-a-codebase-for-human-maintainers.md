...
**2026-09-18 — Stage 0's re-measurement, on `develop` at `707f71a`.** `python3 scripts/maintainability.py measure --totals`: production 43 928 lines of code and 10 068 of comments, tests 67 661 lines in 1 408 tests, 3 fixed sleeps; `cargo nextest list --workspace` runs 1 434. Since the table above, the harness redesign took about 570 lines of production code and 1 950 of tests. The legacy-surface table holds line for line, with one addition: the frontend also carries `shared/dispatch`, `shared/agentActivity`, `shared/orch` and the sprint dispatch dialog, 2 825 lines used from the board, list, table, sprint and item views, so the bridge is in the everyday screens and not only on its own pages. The stale-lease invariant is still in 12 test files. The file lists, with their commands, are in `docs/plans/phase-64.md`. Workspace line coverage is 75.81 % (19 555 of 25 794 lines; `cargo llvm-cov nextest --workspace`, then `cargo llvm-cov report --summary-only`), which sets the single floor of decision 7 at 74.81 %.
