...
**2026-09-18 — The contract shipped, and it differs from this ADR; the adapter is built against what shipped.** docket `0.2.0-beta.2` (2026-09-12) carries `docket harness run|status`, `docs/contracts/harness-v1/schema.json` and four fixtures. Decision 4's gate is therefore met. What was measured in `../rack-cli` and against the installed binary, decision by decision:

- **Decision 10 (prompt on stdin, `--attempt` on argv) holds, by other names.** Harness mode takes `--task TEXT` or `--task-file PATH` and its identity flag is `--agent-id`. Measured against the installed binary: `--task-file /dev/stdin` reads the prompt from stdin, so Tack's stdin delivery works unchanged, nothing is written to disk and nothing reaches `/proc/<pid>/cmdline`. `--task` is never used. The attempt id travels as `--agent-id`.
- **Decision 10 (`reconcile` by pid plus argv token) holds, and turns out to be required.** `docket` is a launcher script that `exec`s a Python interpreter, so `argv[0]` is never the program and a pid check on `argv[0]` alone would call a live docket dead. The shared core accepts the attempt id among the arguments as identity, for every harness. `docket harness status TOKEN` shipped but is not used: the token is printed on stdout, which the runner reads only when the process ends, so it cannot be journaled at start.
- **Decision 6 (capabilities from `docket harness capabilities`).** That command did not ship, and neither did the child-process-group events that `cancel: Supported` would need. v1 probes `docket --version` and declares `cancel: Advisory`, unchanged in substance.

**Decision 5 is resolved differently.** The per-adapter handle encodings are gone — the shared core issues one format. The registry's `<hex kind>:<inner>` wrapping stays, because `reconcile` is given a journal, not a handle, and the journal's only harness-specific field is `process_id`: the kind has to remain recoverable from that string unless the journal format grows a field, which nothing here needs.

The adapter is a `HarnessDescriptor` plus a four-method `HarnessGrammar` on `harness/local_process.rs`, not the five-method trait this ADR's context section describes. Plan and the exact contract mapping: `docs/plans/harnesses.md`. It is built **before** ADR 0068 retires the control-plane bridge, so docket is never without an integration.

**2026-09-18 — routing a mid-run approval to an operator is no longer out of scope (decision 13, third item).** The user decided that pausing to ask is something Tack supports for every harness, not a docket feature: ADR 0068's amendment of the same date keeps the `decisions` path and builds its runner half once, in the harness core. The run stays one subprocess that waits on its own stdin, so nothing has to survive a pause. docket declares `decisions: unsupported` until its harness mode prints a question and reads the answer from stdin; that is asked of docket in `docs/plans/harnesses.md`, and needs no further ADR here. The other two items of decision 13 stand.
