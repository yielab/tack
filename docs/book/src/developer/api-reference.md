...
| Status | Meaning | Schema |
|---|---|---|
| 200 | Orchestration settings: effective enabled flag, where it came from, and reconciler/link counts | — |

#### `PUT /api/settings/orchestration`

save the orchestration enable flag and

**Request body:** `UpdateOrchSettings`

| Status | Meaning | Schema |
|---|---|---|
| 200 | Updated orchestration settings (same shape as GET) | — |

---

## Orchestration

Agent-Factory Control Center: control-plane registration, per-project links, and the Fleet view aggregate. Every route is disabled — 404 — unless TACK_ORCH_ENABLE is set.

#### `GET /api/approvals`

the fleet-wide pending-approval inbox, oldest

| Status | Meaning | Schema |
|---|---|---|
| 200 | Fleet-wide pending-approval inbox, oldest first — includes uncorrelated approvals | `PendingApprovalListResponse` |
| 404 | Orchestration disabled | `ErrorEnvelope` |
...
| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `token` | path | `string` | yes | docket approval token |

**Request body:** `DecideApprovalRequest`

| Status | Meaning | Schema |
|---|---|---|
| 200 | Decision applied — docket's own resulting state | `DecideApprovalResponse` |
| 403 | Missing/invalid X-Tack-Approval-Token header, or TACK_ORCH_APPROVAL_TOKEN not configured on this server | `ErrorEnvelope` |
| 404 | Unknown token, orchestration disabled, or the control plane that issued it was deleted | `ErrorEnvelope` |
| 409 | The approval was already decided (granted/denied/expired) elsewhere | `ErrorEnvelope` |

#### `GET /api/control-planes`

every registered control plane (tokens never

| Status | Meaning | Schema |
|---|---|---|
| 200 | All registered control planes | `ControlPlaneResponse`[] |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |

#### `POST /api/control-planes`

register a control plane.

**Request body:** `CreateControlPlaneRequest`

| Status | Meaning | Schema |
|---|---|---|
| 200 | Control plane registered (token never returned) | `ControlPlaneResponse` |
| 400 | Validation error | `ErrorEnvelope` |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |
...
| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `id` | path | `string` | yes | Control plane ID |

| Status | Meaning | Schema |
|---|---|---|
| 200 | Deleted | — |
| 404 | Not found, or orchestration disabled | `ErrorEnvelope` |
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | The control plane | `ControlPlaneResponse` |
| 404 | Not found, or orchestration disabled | `ErrorEnvelope` |

#### `PATCH /api/control-planes/{id}`

planes/{id}`. Supports optimistic concurrency: an

| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `id` | path | `string` | yes | Control plane ID |

**Request body:** `UpdateControlPlaneRequest`

| Status | Meaning | Schema |
|---|---|---|
| 200 | Updated control plane (token never returned); carries an ETag header naming the new version | `ControlPlaneResponse` |
| 404 | Not found, or orchestration disabled | `ErrorEnvelope` |
| 412 | If-Match did not match the control plane's current version — nothing was written | `ErrorEnvelope` |

#### `GET /api/economics/items`

per-completed-item economics (task 38.1's raw

| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `project_type` | query | `string` | no | Filter to one `project_type` (e.g. `"software"`). Omit for all. |
| `item_type` | query | `string` | no | Filter to one `item_type` (e.g. `"bug"`). Omit for all. |
| `format` | query | `string` | no | `"json"` (default, paginated) or `"csv"` (an attachment; ignores |
| `limit` | query | `integer` | no |  |
| `offset` | query | `integer` | no |  |

| Status | Meaning | Schema |
|---|---|---|
| 200 | Per-completed-item economics (JSON, paginated) or a CSV export attachment, per the format query | `EconomicsItemsResponse` |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | Unit economics (tokens, estimated cost, agent-vs-human lead time, rework rate) sliced by project_type and item_type | `EconomicsSummaryResponse` |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | One row per project linked to a control plane | `FleetListResponse` |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |

#### `GET /api/items/{id}/agent-activity`

every mirrored dispatch attempt and
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | Dispatch attempts and approvals mirrored for this item | `ItemAgentActivityResponse` |
| 404 | Item not found, or orchestration disabled | `ErrorEnvelope` |

#### `POST /api/items/{id}/dispatch`

enqueue a governed task on the item's
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | Dispatch outcome — branch on the `outcome` field, not HTTP status | `DispatchItemResponse` |
| 404 | Item not found, or orchestration disabled | `ErrorEnvelope` |
| 409 | Project not linked to a control plane, a dispatch for this item is already in flight, or the control plane could not be reached | `ErrorEnvelope` |

#### `GET /api/metrics`

Prometheus text exposition merging Tack's own

| Status | Meaning | Schema |
|---|---|---|
| 200 | Prometheus text exposition: Tack's own work-tracking metrics plus the latest mirrored docket sample per metric/label set | — |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |

#### `GET /api/orch-runs/{run_id}`

a pipeline run's state as last mirrored

| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `run_id` | path | `string` | yes | docket's own pipeline-run id, as returned by POST /api/projects/{id}/orch-dispatch |

| Status | Meaning | Schema |
|---|---|---|
| 200 | The run's state as last mirrored by the reconciler; `mirrored: false` if this run id has no mirrored row yet | `OrchRunReadbackResponse` |
| 409 | Orchestration disabled | `ErrorEnvelope` |

#### `GET /api/projects/{id}/agent-activity`

one row per item in this project
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | Latest dispatch-attempt status per item with agent activity | `AgentBadgeResponse` |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | This project's budget cap vs. estimated spend to date | `OrchBudgetResponse` |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |

#### `POST /api/projects/{id}/orch-dispatch`

trigger a full docket pipeline

| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `id` | path | `string` | yes | Project ID |

**Request body:** `DispatchProjectPipelineRequest`

| Status | Meaning | Schema |
|---|---|---|
| 200 | docket accepted the request and started a pipeline run. This reports that the run started, never that it was permitted — poll the reconciler's mirrored run state for the eventual outcome | `DispatchProjectPipelineResponse` |
| 403 | Missing/invalid X-Tack-Dispatch-Token header, or TACK_ORCH_DISPATCH_TOKEN not configured on this server | `ErrorEnvelope` |
| 404 | Project not found, project not linked to a control plane, or orchestration disabled | `ErrorEnvelope` |
| 409 | docket refused the request itself (malformed variables, the control plane unreachable, or — defensively, though not reachable on this route today — a synchronous guardrail refusal) | `ErrorEnvelope` |
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | The project's control-plane link, if any | `OrchLinkView` |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |

#### `PUT /api/projects/{id}/orch-link`

create or replace the project's

| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `id` | path | `string` | yes | Project ID |

**Request body:** `UpsertOrchLinkRequest`

| Status | Meaning | Schema |
|---|---|---|
| 200 | Saved link; carries an ETag header naming the new version | `OrchLinkResponse` |
| 400 | Validation error (e.g. an unknown status name in status_map) | `ErrorEnvelope` |
| 404 | Project or control plane not found, or orchestration disabled | `ErrorEnvelope` |
| 412 | If-Match did not match (or no link exists yet to match) — nothing was written | `ErrorEnvelope` |
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | Guardrail/tool-call/approval metrics for this project's linked control plane (control-plane-wide — see scoped_to_control_plane_only) | `OrchPolicyResponse` |
| 404 | Orchestration disabled (TACK_ORCH_ENABLE unset) | — |

#### `POST /api/sprints/{id}/dispatch`

dispatch every dependency-ready item

| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `id` | path | `string` | yes | Sprint ID |
| `max_in_flight` | path | `['integer', 'null']` | yes | Bound on concurrent HTTP calls to the control plane for this run. |

| Status | Meaning | Schema |
|---|---|---|
| 200 | Sprint dispatch report — one row per item, in dependency order | `SprintDispatchResponse` |
| 404 | Sprint not found, or orchestration disabled | `ErrorEnvelope` |
| 409 | Project not linked to a control plane | `ErrorEnvelope` |

#### `GET /api/sprints/{id}/dispatch/dry-run`

the exact plan a real
...
| Status | Meaning | Schema |
|---|---|---|
| 200 | Dependency-ordered dispatch plan — zero side effects | `DryRunSprintDispatchResponse` |
| 404 | Sprint not found, or orchestration disabled | `ErrorEnvelope` |
| 409 | Project not linked to a control plane | `ErrorEnvelope` |

#### `POST /api/templates/{id}/provision`

create a Tack project from a

| Param | In | Type | Required | Description |
|---|---|---|---|---|
| `id` | path | `string` | yes | Template ID |

**Request body:** `CreateProjectWithPodRequest`

| Status | Meaning | Schema |
|---|---|---|
| 200 | Project created; pod provisioned. `provisioning.status` distinguishes a fully-linked result from one where the pod exists but the link write failed (both are real, neither was rolled back) | `CreateProjectWithPodResponse` |
| 400 | Validation error, or docket refused the provisioning request — the project (if one had been created for this attempt) was rolled back | `ErrorEnvelope` |
| 404 | Template or control plane not found, or orchestration disabled (TACK_ORCH_ENABLE unset) | `ErrorEnvelope` |
| 409 | A pod already exists on this control plane under that remote project name — the project created for this attempt was rolled back | `ErrorEnvelope` |
| 422 | name/description validation failed | `ErrorEnvelope` |
