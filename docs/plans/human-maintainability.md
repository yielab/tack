...
no code changed yet.** Measured 2026-09-11 on `develop` at `f95fbc2`. Every
number comes from `scripts/maintainability.py` (this plan's tool, dry-run only so far) or
from the one-line commands in [Appendix A](#appendix-a--commands-behind-the-numbers); re-run
them before quoting. This plan is workspace-wide; the harness adapters have their own,
...
| Measure | Now | After the mechanical steps (M1–M2) | Target (M8/M9) | Command |
|---|---|---|---|---|
| Production lines (Rust, no inline tests) | 57 453 | 57 453 | — | `measure --totals` |
| Test lines / production lines | 74 014 / **1.29** | 74 014 / 1.29 (moved, not cut) | **≤ 0.8** (≈ 46k) | `measure --totals` |
| Test lines inside `src/*.rs` | 20 882 in 40 modules over budget | **0 over budget** | 0 over 150 | `extract-tests` (dry run) |
| Tests | 1 498 (18.8 s) | 1 498 | ≈ 1 000 | `nextest run --workspace` |
| Comment lines in production | 13 374 (23 %) | ≈ 12 300 (25 preambles moved) | ≤ 20 %, no file > 35 % | `measure` |
| Comment blocks over budget | 149 in 104 files | 124 | 0 | `comment-worklist` |
| Near-identical test names across files | 72 pairs | 72 | 0 | `duplicate-tests` |
| Fixed waits in test code | 74 `sleep(` | 74 | 0 | `measure --totals` |
| Live tests hiding in unit modules | 1 | 0 | 0 | `measure --json` |
| Non-generated docs lines | 91 089 | 91 089 | ≈ 30 000 in tree | Appendix A |
...
8. **No fixed waits.** Poll with a bound or pause time; `docs/adr/0064-fixed-waits.txt` is
   the inventory, regenerated by `scripts/list-fixed-waits.py` — the committed file went
   stale (25 entries recorded, 2 actually remaining, found and fixed 2026-09-12) because no
   card re-ran the generator after editing test code; M8 finishes emptying it.
...
| **M7** | Docs generation per §4: includes in the book, `gen-api-reference.py`, `cargo doc` in CI, the move to `docs/closed-cycles/`, `context-budget.md` rewritten to a quarter of its size because the files it warns about are gone. | human + one agent for the renderer | `mdbook build docs/book` with link check; `regen-generated.sh` idempotent | −60 000 doc lines from the tree |
| **M8** | Close the test-volume gap, added 2026-09-12: M4's per-binary rules landed but measured, only cut ≈ 2 600 of the ≈ 25 000 lines this table expected — 0 `duplicate-tests` pairs, 0 `0064-fixed-waits.txt` entries, ≤ 40-line bodies outside the named exclusions (`wave2_gate.rs`, contract fixtures, live tests, coverage-floor-guarded tests). | agent per crate, largest ratio first | that crate, then the coverage job | measured, not promised — see its handoff for the achieved ratio |
| **M9** | Ratchet down (was M8): set BUDGETS to the per-file targets as hard limits, lock the workspace ratio ceiling at M8's achieved number (0.8 is the aspiration, not a guaranteed landing point once contract-fixture volume is excluded from further cuts), re-baseline, switch `check` from warning on regressions to failing on any file over budget. | human, 1 commit | `check` green | the budgets become the definition of done |
...
5. ~~Whether the workspace ratio target is a hard 0.8 or a measured landing point~~ —
   decided 2026-09-12, after auditing Wave 32 before dispatch: M4 landed correctly against
   its own per-binary rules but, measured, cut ≈ 2 600 of the ≈ 25 000 test lines this plan
   priced it at, so the old M8 (hard-lock at 0.8) would have either failed immediately or
   needed its target quietly loosened. Split into **M8** (a second, realistically-scoped
   volume pass — 0 duplicate tests, 0 fixed waits, ≤ 40-line bodies outside named
   exclusions) and **M9** (the original ratchet-lock, now locking the ratio ceiling at
   whatever M8 actually measures, with any remaining gap to 0.8 named and reasoned rather
   than hidden). See TODO.md's Wave 32 audit note and both cards' text for the detail.
...
git ls-files 'docs/**' | xargs cat | wc -l                                  # 142 581 incl. generated
find docs/agent-handoffs -type f | wc -l; find docs/agent-handoffs -type f | xargs cat | wc -l   # 230 / 59 986
