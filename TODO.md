...
| 7 — Release blocker | III-H1 · H3 · H2 | 57 (cont.) | **III-H1 done (`984bb5f`, on `agent/iii-h1-runner-transport`). III-H3 is next; III-H2 last.** H1 built the runner's HTTP transport — the runner can now register, ask for work, heartbeat, stream progress, upload results and complete against a live server. `./scripts/smoke.sh` passes unmodified through step 6, 2 of 3 harnesses (`codex` not installed here, not counted). 1337 workspace tests / 0 failed, clippy and fmt clean, `runner_contract` 18/18, `openapi_contract` 5/5, `wave2_gate` 5/5. **The board's earlier claim that the transport was the only missing piece was false** — H1 found that the runner still cannot create a working copy of a repository, so a claimed task never actually starts. That is III-H3, created 2026-08-19, and III-H2's release evidence cannot be collected until it lands. H1 also flagged one line outside its ownership (`CapabilityLimits` was not exported from `tack-orch` despite being a public field's type) for the integrator to accept or re-route. |
