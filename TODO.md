...
| 7 — Release blocker | III-H1 · H2 | 57 (cont.) | **open — III-H1 ready to dispatch.** Created after III-G5 refused the tag on an open P0. III-H1 implements `tack-runner`'s HTTP transport (the only missing piece: `UnavailableProtocolClient` is the sole production `RunnerProtocolClient` and `reqwest` is not a dependency). III-H2 then runs the live smoke and tags. H2 is blocked on H1. Base `04ad6f3`. `scripts/smoke.sh` exists now and **fails at the transport step by design** — it is H1's acceptance gate, written before the fix so it is provably load-bearing. Harness reality on this machine: `claude` 2.1.235 and `opencode` 1.18.0 present, `codex` absent. |
