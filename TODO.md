...
| 4 — Fleet scheduling and PM UX | III-E1 · E2 · E3 · E4 · E5 · E6 | 54 | in progress — E1/E2/E5 landed on `plan/harness-agnostic-agent-fleet` at `74fb9db` (deterministic scheduler, shared frontend execution state, CLI/MCP surface); 1105 workspace tests, clippy `-D warnings`, fmt clean; frontend 555 tests + `tsc -b` clean. E3/E4 next (consume E2's merged `frontend/src/shared/execution/**`); E6 integrates last. **Open for E6:** operator execution/fleet/runner/profile routes have empty (`{}`) OpenAPI schemas — no generated types exist for this domain, E2/E5 both hand-typed wire shapes from the Rust handlers; no `GET /api/runners` list endpoint; no operator read path for attempts/events; E1's scheduler is not wired to any real data source yet (no `priority` column on `execution_requests`, `agent_fleets.concurrency_limit` unenforced) — see `III-E1.md`/`III-E2.md`/`III-E5.md` handoffs for the full list |
