...
| 4 — Fleet scheduling and PM UX | III-E1 · E2 · E3 · E4 · E5 · E6 | 54 | in progress — E1/E2/E3/E4/E5 landed on `plan/harness-agnostic-agent-fleet` at `100806f` (deterministic scheduler, shared frontend execution state, fleet/runner management UI, run-with-agent + activity UI, CLI/MCP surface); 1105 workspace tests, clippy `-D warnings`, fmt clean; frontend 653 tests, `tsc -b` and token-lint clean. Only E6 (integration/spec owner) remains. **Open for E6:** operator execution/fleet/runner/profile routes have empty (`{}`) OpenAPI schemas — no generated types exist for this domain, every frontend card hand-typed wire shapes from the Rust handlers; no `GET /api/runners` list endpoint (E3/E4/E5 all hit this independently); no operator read path for attempts/events (`attemptsFor()` always renders a typed "not available" placeholder); E1's scheduler is not wired to any real data source yet (no `priority` column on `execution_requests`, `agent_fleets.concurrency_limit` unenforced); no real "resolved default" provenance source for E4's modal (III-F3 territory, Wave 5) — see `III-E1.md`/`III-E2.md`/`III-E3.md`/`III-E4.md`/`III-E5.md` handoffs for the full list |
