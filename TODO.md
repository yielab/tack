...
| Wave | Cards | Phase | Status |
|---|---|---|---|
| 10 — Standalone single binary | IV-A1 · A2 · A3 · A4 · A5 · A6 | 58 | **IV-A1 through A5 done and integrated** at `ff51bf6` on `develop` (handoffs: `docs/agent-handoffs/part-iv/IV-A1.md` … `IV-A5.md`). `tack runner start` and `tack serve --with-runner` both proven live end-to-end, including zero-touch self-enrollment (A4) with no token ever displayed/copied; `tack runner doctor` (A5) reports honest per-harness presence/version/capabilities, verified byte-identical to a live server's capability snapshot; default `tack serve` starts no runner; non-loopback bind refuses to start `--with-runner`; binary-size delta measured at +0.83 MiB (+4.67%). Full workspace gate green: 1,413 tests, 0 failed. **A6 not started** — last card of the wave, needs A4 (satisfied). Two pre-existing gaps found (not fixed, outside the finding card's ownership): `bootstrap::build_runtime` requires a credential even when a valid stored session exists on disk (worked around, not fixed, in A4's handoff); the embedded runner's log lines are dropped under the server's default `RUST_LOG` filter (A6 should mention broadening it in its operator docs). Working-tree note from §IV.0 (uncommitted `harness/mod.rs` fix) is stale — that fix landed at `f4941f0` during Part V Wave 11 and is already on `develop`. |
