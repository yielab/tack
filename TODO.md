...
| 32 — Close the test-volume gap | IX-M8-dedup, then IX-M8-<crate> ×5 ∥ IX-M6-dev-notes ∥ IX-M7-roadmap | 63 | in progress — IX-M6-dev-notes integrated `4655b6b` (12 notes resolved, directory deleted), IX-M8-dedup integrated `c90bbcc` (49 pairs → 0, all by rename, 0 tests removed; found `tack-api` 68.8 % and `tack-db` 68.3 % already under the 70 % floor on `develop` — CI's coverage job never runs there); IX-M7-roadmap, IX-M8-orch running; runner, api, db, cli next |
