...
mod tests {
    ...
    /// FIPS 180-4 / NIST published test vectors, chosen so this
    /// implementation is checked against an external authority rather than
    /// only against itself.
    #[test]
    ...
    #[test]
    fn handles_input_lengths_that_cross_a_block_boundary() {
        // 55, 56 and 64 bytes exercise the padding edge cases (message +
        // 0x80 + length either fits in the current 64-byte block or spills
        // into a second one).
        for length in [0usize, 55, 56, 57, 64, 119, 120, 121, 1000] {
    ...
    #[test]
    fn distinct_input_produces_distinct_digests() {
