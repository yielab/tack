...
pub async fn import_linear(
    ...
    loop {
        ...
        for issue in &conn.nodes {
            ...
            let priority = linear_priority_to_tack(issue.priority);
...
fn linear_priority_to_tack(p: Option<u8>) -> Option<tack_core::models::Priority> {
    use tack_core::models::Priority;
    match p {
...
mod tests {
    ...
    fn priority_mapping() {
        use tack_core::models::Priority;
        assert_eq!(linear_priority_to_tack(Some(1)), Some(Priority::Critical));
        assert_eq!(linear_priority_to_tack(Some(2)), Some(Priority::High));
        assert_eq!(linear_priority_to_tack(Some(3)), Some(Priority::Medium));
