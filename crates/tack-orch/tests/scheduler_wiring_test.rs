...
use tack_db::repo::execution::{NewAgentProfile, NewExecutionRequest, NewRunner};
...
async fn setup_repo() -> (Repository, String) {
    ...
    sqlx::query("INSERT INTO workspaces (id, name, default_vocabulary) VALUES (?, 'Wiring', ?)")
    ...
    let item = repo
        .create_item(
            ...
            &status,
            CreateItem {
                title: "Scheduler wiring proof".into(),
                description: None,
