...
async fn list_sends_get_items() {
    ...
    let resp = run_blocking(move || {
        let config = make_config(&uri);
        FlexpmClient::new(&config).unwrap().get(&path)
    })
    .await;
...
async fn search_global_sends_correct_path() {
    ...
    let resp = run_blocking(move || {
        let config = make_config(&uri);
        FlexpmClient::new(&config).unwrap().get("/search?q=login")
    })
    .await;
...
async fn bearer_token_is_forwarded() {
    ...
    let resp = run_blocking(move || {
        ...
        FlexpmClient::new(&config).unwrap().get("/projects")
    })
    .await;
...
fn config_save_and_reload() {
    ...
    let tmp = std::env::temp_dir().join(format!("flexpmrc_test_{}", std::process::id()));
...
async fn vocab_fetch_returns_empty_on_404() {
    ...
    let map = run_blocking(move || {
        let config = make_config(&uri);
        let client = FlexpmClient::new(&config).unwrap();
