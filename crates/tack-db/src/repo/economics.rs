...
//! `orch_events` for the rework signal). Its own module rather than an
//! extension of `repo/orch.rs`, whose tables these additive/read-only queries
//! reuse, so edits here never collide with unrelated edits there.
//!
//! The rework-signal event types (`rework_started`, `verification_failed`,
//! `tester_verdict_failed`) arrive only via the reconciler's trace ingestion,
//! which always leaves `orch_events.run_id` unset (docket's payload carries no
//! `run_id`, only `session_id`) — so [`Repository::list_item_ids_with_rework_signal`]
//! correlates at the item level instead of per-attempt; a disclosed gap.
//!
//! Only `orch_events`/`orch_metrics` are retention-swept — `orch_tasks` never
//! is, so token/cost/lead-time figures are never truncated, but the
//! rework-signal correlation can silently miss history once its raw events
//! age out. Callers must compare `last_dispatched_at` against their own
//! retention cutoff before treating absence as "confirmed no rework."
...
pub struct ItemEconomicsRow {
    ...
    /// Latest `orch_tasks.dispatched_at` — used only to judge whether this
    /// item's rework-signal correlation is trustworthy (see module doc).
