...
pub async fn seed_builtin_templates(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    use flexpm_core::{vocabulary::vocabulary_for_type, workflow::workflow_for_type};
    ...
    for spec in specs {
        ...
        let columns: Vec<flexpm_core::models::BoardColumn> = workflow
            .statuses
            .iter()
            .map(|s| flexpm_core::models::BoardColumn {
                status: s.name.clone(),
        ...
        let default_boards = vec![flexpm_core::models::BoardTemplate {
