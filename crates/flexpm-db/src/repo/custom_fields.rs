...
#[instrument(skip(pool))]
pub async fn get_field(pool: &SqlitePool, id: Uuid) -> Result<CustomFieldDefinition, sqlx::Error> {
    ...
    Ok(CustomFieldDefinition {
        ...
        project_id: row
            .project_id
            .as_ref()
            .map(|pid| Uuid::parse_str(pid).unwrap()),
        name: row.name,
        field_type,
...
pub async fn update_field(
    ...
    if let Some(description) = &data.description {
        sqlx::query(
            "UPDATE custom_field_definitions SET description = ?, updated_at = ? WHERE id = ?",
        )
        .bind(description)
        .bind(now.to_rfc3339())
        .bind(id.to_string())
    ...
    if let Some(required) = data.required {
        sqlx::query(
            "UPDATE custom_field_definitions SET required = ?, updated_at = ? WHERE id = ?",
        )
    ...
    if let Some(default_value) = data.default_value {
        ...
        sqlx::query(
            "UPDATE custom_field_definitions SET default_value = ?, updated_at = ? WHERE id = ?",
        )
        .bind(value_str)
        .bind(now.to_rfc3339())
        .bind(id.to_string())
    ...
    if let Some(validation) = data.validation {
        ...
        sqlx::query(
            "UPDATE custom_field_definitions SET validation = ?, updated_at = ? WHERE id = ?",
        )
        .bind(validation_str)
        .bind(now.to_rfc3339())
        .bind(id.to_string())
...
#[instrument(skip(pool))]
pub async fn delete_field(pool: &SqlitePool, id: Uuid) -> Result<(), sqlx::Error> {
