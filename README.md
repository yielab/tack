...
Grab the archive for your system from the [**releases page**](https://github.com/yielab/tack/releases), extract it, and run the server — everything (UI, API, database engine) is inside one file.

> Tack is currently in **beta** — these builds are published for testing. If something breaks, please [open an issue](https://github.com/yielab/tack/issues).
